...
def _trade_history_rows(trade_log: list[dict], n: int = 50) -> str:
    rows = ""
    for t in reversed(trade_log[-n:]):
        date = t.get("date", "")
        ...
        pnl_cell = (
            f"<td style='color:{'#2ecc71' if pnl >= 0 else '#e74c3c'}'>Rs.{pnl:+,.2f}</td>"
            if pnl is not None else "<td>&mdash;</td>"
        )
        rows += (
            f"<tr><td>{date_str}</td><td>{t.get('ticker', '').replace('.NS', '')}</td>"
            f"<td style='color:{action_color}; font-weight:bold'>{action}</td>"
            f"<td>{t.get('qty', '')}</td><td>Rs.{t.get('price', 0):,.2f}</td>"
            f"<td>Rs.{t.get('value', 0):,.2f}</td>{pnl_cell}</tr>"
        )
    return rows
...
def generate_daily_report(
    ...
    trade_rows = _trade_history_rows(portfolio_state.get("trade_log", []))
    ...
    trade_history_block = (
        "<p style='color:#888'>No trades yet.</p>" if not trade_rows else
        f"<table><tr><th>Date</th><th>Ticker</th><th>Action</th><th>Qty</th>"
        f"<th>Price</th><th>Value</th><th>Realised P&amp;L</th></tr>{trade_rows}</table>"
    )
...
<div class="section" style="font-size:0.75rem; color:#555;">
  ...
  <p style="margin-top:8px;">
    <a href="backtest/technical.html" style="color:#00CED1;">Multi-year backtest (momentum/low-vol)</a> &middot;
    <a href="backtest/full.html" style="color:#00CED1;">Backtest incl. fundamentals</a>
  </p>
</div>
</body>
</html>"""
